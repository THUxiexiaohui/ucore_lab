...
static int
init_main_sleep(void *arg) {
    cprintf(" kernel_thread, pid = %d, name = %s\n", current->pid, get_proc_name(current));
	schedule();
    cprintf(" kernel_thread, pid = %d, name = %s , arg  %s \n", current->pid, get_proc_name(current), (const char *)arg);
	schedule();
    cprintf(" kernel_thread, pid = %d, name = %s , I am still alive :)\n",current->pid, get_proc_name(current));
	schedule();
    cprintf(" kernel_thread, pid = %d, name = %s , I am still alive, en.... \n");
	schedule();
    cprintf(" kernel_thread, pid = %d, name = %s , I am still alive, en........ \n");
	schedule();
    cprintf(" kernel_thread, pid = %d, name = %s , I am done, Bye, Bye. :) \n");
    return 0;
}
...
proc_init(void) {
    ...
    int pid3= kernel_thread(init_main_sleep, "init main3: long-long-life Tread!!", 0);
    if (pid1 <= 0 || pid2<=0) {
